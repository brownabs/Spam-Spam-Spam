...
// You will need a nested array method - meaning one iteration inside another one - since you need to 
// iterate the entire array of customers, and then iterate the array of emails for each one.
...
let customers = [
  ...
  const emailAccounts = customers.map(customer => {//using the filter method ... passes a condition
    const customerEmailArray = []
    const emails = customer.contacts.email
    customerEmailArray.push(emails)
